...
class MatrixTransposer {
 ...
 public:
  ...
  static void Transpose(unsigned int* rank_end_offsets, T*& cells,
    ...
    MPI_Allreduce(&my_row_count, &total_row_count, 1, MPI_UNSIGNED, MPI_SUM,
                  MPI_COMM_WORLD);
    ...
    for (unsigned int n = 0; n < (unsigned int)total_row_count; n++) {
      ...
      while (cpu_id < static_cast<unsigned int>(mpi_size) &&
             rank_end_offsets[cpu_id] <= n) {
        cpu_id++;
      }
    }
    ...
    // 2nd step: now we will send the respective table info to each CPU
    // (qsortStruct)
    ...
    for (int cpu = 0; cpu < mpi_size; cpu++) sendcounts[cpu] = 0;
    ...
    for (int cpu = 0; cpu < mpi_size; cpu++)
      sentdispls[cpu] =
          cpu == 0 ? 0 : sentdispls[cpu - 1] + sendcounts[cpu - 1];

    for (int cpu = 0; cpu < mpi_size; cpu++)
      recvdispls[cpu] =
          cpu == 0 ? 0 : recvdispls[cpu - 1] + recvcounts[cpu - 1];
    ...
    // we sort by row, not by cpu (therefore converting into a single sparse
    // matrix)
    SortCellsByCpuRowColumn(my_col_count, total_cell_count, metadatas, cells);
