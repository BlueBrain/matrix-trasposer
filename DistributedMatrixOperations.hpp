...
private:
  ...
  inline static int qsortCompare (const void * _a, const void * _b)
  {
    QSortWrapper * a = static_cast<QSortWrapper*> ((void*)_a);
    QSortWrapper * b = static_cast<QSortWrapper*> ((void*)_b);
  ...
  inline static void sortCellsByCpuRowColumn(unsigned int myColumnsCount, unsigned int totalCellsCount, QSortWrapper * qsortWrapper, T*& cells)
  {
...
public:
  ...
  inline static void sparseMatrixTranspose(unsigned int * endRowIndexPerCpu, unsigned int *& numberOfColumnsPerRow, unsigned int *& columnsIndex, unsigned int *& numberOfCellsPerColumn, T *& cells, MPI_Comm mpiComm)
  {
    //MPI variables
    ...
    for (unsigned int n=0; n< (unsigned int) totalRowsCount; n++)
    {
        ...
        while (cpuId < static_cast<unsigned int>(mpiSize) && endRowIndexPerCpu[cpuId] <= n){
            cpuId++;
        }
    }
